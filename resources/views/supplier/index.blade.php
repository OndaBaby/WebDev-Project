...
@section('content')
    ...
    <div class="container">
        ...
        @endif
        <a class="btn btn-primary mb-3" href="{{ route('supplier.create') }}">Add Supplier</a>
        <table class="table table-striped">
            <thead>
            <tr>
                <th scope="col">Name</th>
            ...
            @foreach ($suppliers as $supplier)
                <tr>
                    ...
                    <td>
                        <div class="carousel" id="carousel{{ $supplier->id }}">
                            <div class="carousel-inner">
                                @foreach (explode(',', $supplier->img_path) as $key => $imgPath)
                                    <div class="carousel-item {{ $key === 0 ? 'active' : '' }}">
                                        <img src="{{ asset($imgPath) }}" alt="Supplier Image {{ $key + 1 }}" class="d-block w-100 product-image">
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <td>
                        <div class="d-flex">
                            <a href="{{ route('supplier.edit', $supplier->id) }}" class="btn btn-sm btn-primary mr-1"><i class="fas fa-edit"></i> Edit</a>
                            @if ($supplier->deleted_at === null)
                                <form action="{{ route('supplier.delete', $supplier->id) }}" method="POST" class="delete-form">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i> Delete</button>
                                </form>
                            @else
                                <form action="{{ route('supplier.restore', $supplier->id) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-success"><i class="fas fa-undo"></i> Restore</button>
                                </form>
                            @endif
